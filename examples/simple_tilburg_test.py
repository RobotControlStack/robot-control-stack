...
if __name__ == "__main__":
    ...
    for i in range(len(config["motor_calib_min_range_deg"])):
        joint_angle = power_grasp_values[i] * (config["motor_calib_max_range_deg"][i] - config["motor_calib_min_range_deg"][i]) + config["motor_calib_min_range_deg"][i]
    ...
    config = THConfig(calibration_file="/home/sbien/Documents/Development/RCS/robot-control-stack/python/rcs/hand/calibration_og.json", grasp_percentage=1, hand_orientation="right")

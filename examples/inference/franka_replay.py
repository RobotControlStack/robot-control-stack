...
class ReplayConfig:
    ...
    # Where the per-frame replay CSV + meta are written (None disables).
...
def main() -> None:
    rcfg = ReplayConfig()
    cfg = InferenceConfig()  # inherits gains, TAM flags, warmup, fps
    cfg.tam = rcfg.tam  # replay decides TAM on/off independently of the policy default

    if rcfg.log_dir is None:
        rcfg.log_dir = os.path.join(os.getcwd(), "replay_logs", time.strftime("%Y%m%d_%H%M%S"))
    os.makedirs(rcfg.log_dir, exist_ok=True)
    logger.info("replay logs -> %s", rcfg.log_dir)
    ...
    replay_log = None
    if rcfg.log_dir:
        replay_log = open(os.path.join(rcfg.log_dir, "replay_frames.csv"), "w")
        ...
        with open(os.path.join(rcfg.log_dir, "replay_meta.json"), "w") as f:
            json.dump(
                {
    ...
    with env:
        ...
        try:
            ...
            if replay_log is not None:
                replay_log.close()
            env.reset()
            logger.info("replay logs in %s", rcfg.log_dir)

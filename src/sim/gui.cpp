...
static size_t calculate_state_size(mjModel const* m, mjtState spec) {
  return mj_stateSize(m, spec);
}
...
static size_t calculate_shm_size(mjModel const* m, mjData const* d,
  ...
  size_t const total_required_size =
      extra_info_size + calculate_mdl_size(m) +
      calculate_state_size(m, state_spec) * sizeof(mjtNum);
...
GuiServer::GuiServer(Sim& sim, const std::string& id)
  ...
  mj_getState(sim.m, sim.d, this->shm.state.ptr, mjSTATE_FULLPHYSICS);
...
void GuiServer::update_mjdata_callback() {
  if (*this->shm.info_byte & UPDATE_SIM_STATE) {
    mj_getState(this->sim.m, this->sim.d, this->shm.state.ptr,
                mjSTATE_FULLPHYSICS);
...
GuiClient::GuiClient(const std::string& id)
  ...
  mj_setState(this->m, this->d, this->shm.state.ptr, mjSTATE_FULLPHYSICS);

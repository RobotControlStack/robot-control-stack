...
void Sim::start_gui_server(const std::string& id) {
  ...
  this->register_cb(std::bind(&GuiServer::update_mjdata_callback, &this->gui.value()), 0);
}
// TODO: when stop_gui_server is called, the callback still exists but now points to an non existing gui
void Sim::stop_gui_server() {
  this->gui.reset();
}
}  // namespace sim
}  // namespace rcs

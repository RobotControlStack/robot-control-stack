...
common::Pose FR3::world_pose_to_robot_pose(const common::Pose &pose) {
  return this->cfg.world_to_robot.has_value() ? this->cfg.world_to_robot.value().inverse() * pose : pose;
}

common::Pose FR3::robot_pose_to_world_pose(const common::Pose &pose) {
  return this->cfg.world_to_robot.has_value() ? pose.inverse() * this->cfg.world_to_robot.value() : pose;
}

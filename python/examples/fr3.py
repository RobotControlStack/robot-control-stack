...
def main():
    ...
    # 5cm in x direction
    robot.set_cartesian_position(robot.get_cartesian_position() * rcsss.common.Pose(translation=np.array([0.05, 0, 0])))
    if ROBOT_INSTANCE == RobotInstance.SIMULATION:
        simulation.step_until_convergence()
    ...
    # 5cm in y direction
    robot.set_cartesian_position(robot.get_cartesian_position() * rcsss.common.Pose(translation=np.array([0, 0.05, 0])))
    if ROBOT_INSTANCE == RobotInstance.SIMULATION:
        simulation.step_until_convergence()
        logger.debug(f"IK success: {robot.get_state().ik_success}")
        logger.debug(f"sim converged: {simulation.is_converged()}")
    ...
    # 5cm in z direction
    robot.set_cartesian_position(robot.get_cartesian_position() * rcsss.common.Pose(translation=np.array([0, 0, 0.05])))
    if ROBOT_INSTANCE == RobotInstance.SIMULATION:
        simulation.step_until_convergence()
        logger.debug(f"IK success: {robot.get_state().ik_success}")
        logger.debug(f"sim converged: {simulation.is_converged()}")
    ...
    robot.set_cartesian_position(robot.get_cartesian_position() * rcsss.common.Pose(translation=np.array([0, 0, 0.25])))
    if ROBOT_INSTANCE == RobotInstance.SIMULATION:
        simulation.step_until_convergence()
        logger.debug(f"IK success: {robot.get_state().ik_success}")
        logger.debug(f"sim converged: {simulation.is_converged()}")
